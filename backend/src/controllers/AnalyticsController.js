...
class AnalyticsController {
  ...
  static async getSummary(req, res) {
    try {
      ...
      Logger.error('AnalyticsSummary error', { error: error.message });
      res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  static async getSentimentTrend(req, res) {
    try {
      ...
      Logger.error('SentimentTrend error', { error: error.message });
      res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  static async getTopKeywords(req, res) {
    try {
      ...
      Logger.error('TopKeywords error', { error: error.message });
      res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  static async getDashboard(req, res) {
    try {
      ...
      const [summary, trend, keywords] = await Promise.all([
        SentimentAnalysisService.getAnalyticsSummary(days),
        SentimentAnalysisService.getSentimentTrend(days),
        SentimentAnalysisService.getTopKeywords(null, 10)
      ]);
