...
class ChatAIController {
  ...
  async aiReply(req, res) {
    ...
    try {
      ...
      try {
        ...
        this.logger.debug('👤 User processed', {
          userId: user.id,
      ...
      try {
        ...
        aiResponseData = await this.chatAIService.generateAIResponse(userMessage, conversationHistory);
        ...
        this.logger.error('❌ RAG AI generation failed', {
          error: error.message
        });
        aiResponseData = {
  ...
  async getUsers(req, res) {
    try {
      ...
      this.logger.error('Error getting ChatAI users', {
        error: error.message
      });
      res.status(500).json({
        success: false,
  ...
  async getResponses(req, res) {
    try {
      ...
      this.logger.error('Error getting ChatAI responses', {
        error: error.message
      });
      res.status(500).json({
        success: false,
  ...
  async addResponse(req, res) {
    try {
      ...
      this.logger.error('Error adding ChatAI response', {
        error: error.message
      });
      res.status(500).json({
        success: false,
  ...
  async testAI(req, res) {
    try {
      ...
      this.logger.error('Error testing ChatAI', {
        error: error.message
      });
      res.status(500).json({
        success: false,
  ...
  async getVectorDatabaseInfo(req, res) {
    try {
      ...
      this.logger.error('Error getting vector database info', {
        error: error.message
      });
      res.status(500).json({
        success: false,
    ...
    try {
      ...
      res.json({
        ...
        results: relevantDocs.map(doc => ({
          ...
          snippet: doc.type === 'post' ? doc.content?.substring(0, 200) : doc.type === 'response' ? doc.responseText?.substring(0, 200) : doc.summary
        })),
        count: relevantDocs.length
      });
    } catch (error) {
      this.logger.error('Error searching similar documents', {
        error: error.message
      });
      res.status(500).json({
        success: false,
  ...
  async getPostsForAnalysis(req, res) {
    try {
      ...
      this.logger.error('Error getting posts for analysis', {
        error: error.message
      });
      res.status(500).json({
        success: false,
