...
async function checkMySQLIndexes() {
  ...
  try {
    ...
    const sortedTables = Object.entries(tableIndexes).sort(([, a], [, b]) => b.count - a.count).slice(0, 5);
    ...
    const exceededTables = Object.entries(tableIndexes).filter(([, data]) => data.count > 64);
