...
async function generateResponse(userMessage, systemPrompt, chatHistory = []) {
  try {
    ...
    const fullPrompt = systemPrompt ? `${vietnameseInstruction}${systemPrompt}\n\nTin nhắn từ khách hàng: ${userMessage}` : `${vietnameseInstruction}Tin nhắn từ khách hàng: ${userMessage}`;
    ...
    // Call Gemini API
    const response = await axios.post(`${GEMINI_API_URL}?key=${apiKey}`, requestPayload, {
      headers: {
