    ...
    try {
      ...
      try {
        if (post_id) {
          ...
          const platform = typeof status === 'string' && status.includes(':') ? status.split(':')[1] : null;
          ...
          await PlatformPost.create({
            ...
            platform: platform || (Array.isArray(parentPost?.platform) ? parentPost.platform[0] : typeof parentPost?.platform === 'string' ? function (p) {
              try {
                ...
                return p.includes(',') ? p.split(',')[0].trim() : p;
              }
  ...
  async getPostsByStatus(status) {
    try {
      ...
      const mapped = posts.map(p => {
        ...
        let platforms = [];
        try {
          if (!p.platform) platforms = [];else if (Array.isArray(p.platform)) platforms = p.platform;else if (typeof p.platform === 'string') {
