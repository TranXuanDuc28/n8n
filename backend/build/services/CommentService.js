...
const {
  generateResponse
} = require('../config/gemini'); // Đảm bảo file gemini.js là bản đã sửa
...
class CommentService {
    ...
    try {
      ...
      for (const comment of commentsData) {
        try {
          ...
          const isHandled = await HandledComment.findOne({
            where: {
          ...
          const processingResult = await SentimentAnalysisService.processComment(comment.comment_id, comment.message);
          if (!processingResult.shouldReply) {
            let reason = 'Unknown';
            if (processingResult.isToxic) reason = `Toxic detected (${processingResult.analysis?.toxicCategory})`;else if (processingResult.isSpam) reason = 'Spam detected';
            results.skipped.push({
              comment_id: comment.comment_id,
              reason,
          ...
          // 🧠 Sinh phản hồi từ AI
          const aiResult = await this.generateAIResponse(comment.message, comment.from_name, comment.from_id, comment.post_id, sessionId);
      ...
      Logger.error('ProcessComments error', {
        error: error.message
      });
      return {
        success: false,
  ...
  static async generateAIResponse(userMessage, userName, userId, postId, sessionId, workflowData = {}) {
    try {
      ...
      if (!aiResult.success || !aiResult.response) {
        ...
        Logger.warn('⚠️ Gemini trả về rỗng, dùng fallback message.', {
          userMessage
        });
        return {
          success: true,
  ...
  static async markCommentsHandled(handledData) {
    try {
      ...
      Logger.error('MarkCommentsHandled error', {
        error: error.message
      });
      return {
        success: false,
  ...
  static async checkSingleCommentHandled(commentId, sessionId) {
    try {
      ...
      Logger.error('CheckSingleCommentHandled error', {
        error: error.message
      });
      return {
        success: false,
  ...
  static async checkHandledStatus(commentIds) {
    try {
      const handled = await HandledComment.findAll({
        where: {
      ...
      Logger.error('CheckHandledStatus error', {
        error: error.message
      });
      return {
        success: false,
  ...
  static async getUnhandledComments(limit = 100) {
    try {
      ...
      Logger.error('GetUnhandledComments error', {
        error: error.message
      });
      return {
        success: false,
