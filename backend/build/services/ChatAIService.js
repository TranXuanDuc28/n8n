...
class ChatAIService {
  constructor() {
    ...
    this.model = this.gemini ? this.gemini.getGenerativeModel({
      model: "gemini-2.5-flash"
    }) : null;
    this.embeddingModel = this.gemini ? this.gemini.getGenerativeModel({
    ...
    this.ragConfig = {
      topK: 7,
      // Số lượng tài liệu liên quan nhất được trả về
  ...
  async buildVectorDatabase() {
    try {
      ...
      if (this.vectorCache.lastUpdate && now - this.vectorCache.lastUpdate < this.ragConfig.cacheExpiry && this.vectorCache.posts.length > 0) {
      ...
      for (const post of recentPosts) {
        try {
          ...
          if (post.platformPosts && Array.isArray(post.platformPosts)) {
            const platformTexts = post.platformPosts.map(pp => pp.content).filter(Boolean).join('\n');
            fullText += `\n${platformTexts}`;
          }
      ...
      for (const response of staticResponses) {
        try {
          const embedding = await this.generateEmbedding(`${response.keyword} ${response.response_text}`);
      ...
      this.logger.error('Error building vector database', {
        error: error.message
      });
      throw error;
    }
  ...
  async retrieveRelevantDocuments(query) {
    try {
      ...
      const postFiltered = postScores.filter(p => p.similarity >= threshold).sort((a, b) => b.similarity - a.similarity);
      const abTestFiltered = abTestScores.filter(a => a.similarity >= threshold).sort((a, b) => b.similarity - a.similarity);
      const responseFiltered = responseScores.filter(r => r.similarity >= threshold).sort((a, b) => b.similarity - a.similarity);
      ...
      this.logger.error('Error retrieving relevant documents', {
        error: error.message
      });
      return [];
    }
  ...
  buildRAGContext(relevantDocs) {
    ...
    if (posts.length > 0) {
      ...
      posts.forEach((post, idx) => {
        contextParts.push(`${idx + 1}. "${post.title}" (độ liên quan: ${(post.similarity * 100).toFixed(1)}%)`, `   Nội dung: ${post.content?.substring(0, 200) || post.fullText?.substring(0, 200)}...`, `   Engagement: ${post.engagementScore || 0} tương tác`);
      });
    }
    ...
    if (responses.length > 0) {
      ...
      responses.forEach((resp, idx) => {
        contextParts.push(`${idx + 1}. Từ khóa: "${resp.keyword}" (độ liên quan: ${(resp.similarity * 100).toFixed(1)}%)`, `   Phản hồi: ${resp.responseText}`);
      });
    }
    ...
    if (abTests.length > 0) {
      ...
      abTests.forEach((test, idx) => {
        contextParts.push(`${idx + 1}. Test "${test.testType}" (độ liên quan: ${(test.similarity * 100).toFixed(1)}%)`, `   Kết quả: ${test.summary}`, `   Chi tiết: ${test.detailText}`);
        if (test.bestVariant) {
          const metrics = test.bestVariant.metrics || {};
          contextParts.push(`   Variant tốt nhất: ${metrics.likes || 0} likes, ${metrics.comments || 0} comments, ${metrics.shares || 0} shares`);
        }
      });
    }
  ...
  async generateAIResponse(message, conversationHistory = [], databaseResponses = []) {
    ...
    try {
      ...
      const conversationContext = conversationHistory.slice(-5) // Chỉ lấy 5 tin nhắn gần nhất
      .map(conv => `${conv.message_type === 'received' ? 'User' : 'Assistant'}: ${conv.message_text}`).join('\n');
      ...
      this.logger.error('Error generating RAG AI response', {
        error: error.message
      });
      throw error;
    }
  ...
  async refreshVectorDatabase() {
    try {
      ...
      this.logger.error('Error refreshing vector database', {
        error: error.message
      });
      throw error;
    }
  ...
  async getOrCreateUser(facebookId, userProfile = {}) {
    try {
      let user = await ChatAIUser.findOne({
        where: {
      ...
      if (!user) {
        ...
        this.logger.info('Created new ChatAI user', {
          userId: user.id,
          facebookId
        });
      } else {
        ...
        this.logger.info('Updated existing ChatAI user', {
          userId: user.id,
          facebookId
        });
      }
      ...
      this.logger.error('Error getting/creating ChatAI user', {
        error: error.message,
        facebookId
      });
      throw error;
  ...
  async saveMessage(userId, messageText, messageType, facebookMessageId = null, conversationId = null) {
    try {
      ...
      this.logger.error('Error saving ChatAI conversation', {
        error: error.message,
        userId
      });
      throw error;
  ...
  async getConversationHistory(userId, limit = 10) {
    ...
    try {
      ...
      this.logger.error('Error getting ChatAI conversation history', {
        error: error.message,
        userId
      });
      return [];
  ...
  async logAnalytics(userId, eventType, eventData = {}) {
    try {
      ...
      this.logger.debug('Logged ChatAI analytics', {
        userId,
        eventType
      });
    } catch (error) {
      this.logger.error('Error logging ChatAI analytics', {
        error: error.message,
        userId
      });
    }
  ...
  async getAnalytics(startDate = null, endDate = null) {
    try {
      ...
      this.logger.error('Error getting ChatAI analytics', {
        error: error.message
      });
      return {
        total: 0,
  ...
  async getStats() {
    try {
      const [userCount, conversationCount, responseCount, publishedPostsCount] = await Promise.all([ChatAIUser.count(), ChatAIConversation.count(), ChatAIResponse.count({
        where: {
          is_active: true
        }
      }), Post.count({
      ...
      this.logger.error('Error getting ChatAI stats', {
        error: error.message
      });
      return {
        users: 0,
