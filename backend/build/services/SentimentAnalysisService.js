...
class SentimentAnalysisService {
  ...
  static async analyzeSentiment(text) {
    try {
      ...
      const keywords = await SentimentKeyword.findAll({
        attributes: ['keyword', 'sentiment', 'weight'],
        raw: true
      });
      let positiveScore = 0;
      let negativeScore = 0;
  ...
  static async getSentimentTrend(days = 30) {
    try {
      ...
      rows.forEach(r => {
        ...
        byDate[key][r.sentiment] = byDate[key][r.sentiment] || {
          count: 0,
          totalScore: 0
        };
      ...
      Object.entries(byDate).forEach(([date, sentiments]) => {
        Object.entries(sentiments).forEach(([sentiment, v]) => {
          results.push({
            ...
            sentiment,
            count: v.count,
            avg_score: v.count ? v.totalScore / v.count : 0
          });
        });
      });
      return {
  ...
  static async getTopKeywords(sentiment = null, limit = 20) {
    try {
      ...
      const topKeywords = Object.values(keywordMap).sort((a, b) => b.frequency - a.frequency).slice(0, limit);
      return {
        success: true,
        data: topKeywords
      };
