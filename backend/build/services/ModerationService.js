    ...
    try {
      ...
      try {
        const capabilities = await axios.get(url, {
          params: {
            fields: 'can_hide,is_hidden,from,permalink_url'
          },
          headers: {
        ...
        Logger.debug('Comment capabilities', {
        ...
        if (is_hidden === true) {
          Logger.info('Comment already hidden', {
            comment_id: commentId
          });
          return {
            success: true,
        ...
        if (can_hide === false) {
          Logger.warning('Comment cannot be hidden by this token/page', {
            comment_id: commentId,
            from,
            permalink_url
          });
      ...
      Logger.info('Comment hidden successfully', {
        comment_id: commentId
      });
      return {
        success: true,
