...
module.exports = (sequelize) => {
  const CommentAnalysis = sequelize.define('CommentAnalysis', {
    ...
    comment_id: { type: DataTypes.STRING(100), allowNull: false, unique: true },
    original_message: { type: DataTypes.TEXT, allowNull: false },
    cleaned_message: { type: DataTypes.TEXT, allowNull: true },
    is_spam: { type: DataTypes.BOOLEAN, defaultValue: false },
    is_duplicate: { type: DataTypes.BOOLEAN, defaultValue: false },
    duplicate_of: { type: DataTypes.STRING(100), allowNull: true },
    message_length: { type: DataTypes.INTEGER, allowNull: true },
    word_count: { type: DataTypes.INTEGER, allowNull: true },
    has_emoji: { type: DataTypes.BOOLEAN, defaultValue: false },
  ...
  CommentAnalysis.associate = (models) => {
    CommentAnalysis.belongsTo(models.FacebookComment, { foreignKey: 'comment_id', targetKey: 'comment_id', as: 'facebookComment' });
  };
