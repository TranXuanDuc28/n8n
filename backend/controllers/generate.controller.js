...
class GenerateController {
  ...
  static async generateContent(req, res) {
    try {
      const { prompt_image} = req.body;
      if (!prompt_image) {
        return res.status(400).json({ success: false, message: "prompt_image is required" });
      }

      const payload = { prompt: prompt_image, response_format: 'b64_json', model: 'black-forest-labs/FLUX.1-schnell' };
