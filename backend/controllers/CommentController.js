...
class CommentController {
  ...
  static async processComments(req, res) {
    try {
      const { comments, session_id } = req.body;
      ...
      Logger.error('ProcessComments controller error', { error: error.message });
      res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  static async markHandled(req, res) {
    try {
      const { handled_comments } = req.body;
      ...
      Logger.error('MarkHandled controller error', { error: error.message });
      res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  static async checkHandled(req, res) {
    try {
      const { comment_id, session_id } = req.body;
      ...
      Logger.error('CheckHandled controller error', { error: error.message });
      res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  static async getUnhandled(req, res) {
    try {
      ...
      Logger.error('GetUnhandled controller error', { error: error.message });
      res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  static async savePosts(req, res) {
    try {
      ...
      Logger.error('SavePosts controller error', { error: error.message });
      res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  static async generateResponse(req, res) {
    try {
      const { message, user_name, user_id, post_id, session_id, workflow_data } = req.body;
      ...
      const sessionId = session_id || `session_${Date.now()}`;
      const result = await CommentService.generateAIResponse(
        message,
        user_name || 'User',
        user_id || 'unknown',
        post_id || '',
      ...
      Logger.error('GenerateResponse controller error', { error: error.message });
      res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  static async processTemplate(req, res) {
    try {
      const { prompt_name, workflow_data } = req.body;
      ...
      Logger.error('ProcessTemplate controller error', { error: error.message });
      res.status(500).json({
        success: false,
        error: error.message
      });
